...
def run(args, current_time, device):
    ...
    if args.mode == 'clean':
        ...
        if args.debug:
            run_fair(fold=0, train_df=train_df, male_df=male_df, female_df=female_df, test_df=test_df, args=args,
                     device=device,
                     current_time=current_time)
        else:
            for fold in range(args.folds):
                run_fair(fold=fold, train_df=train_df, male_df=male_df, female_df=female_df, test_df=test_df, args=args,
                         device=device,
                         current_time=current_time)
        ...
        # fold, train_df, test_df, male_df, female_df, args, device, current_time)
        if args.debug:
            run_fair_dpsgd_track_grad(fold=0, train_df=train_df, test_df=test_df, male_df=male_df, female_df=female_df,
                                      args=args,
                                      device=device,
                                      current_time=current_time)
        else:
            for fold in range(args.folds):
                run_fair_dpsgd_track_grad(fold=fold, male_df=male_df, female_df=female_df, test_df=test_df,
                                          train_df=train_df,
                                          args=args, device=device,
                                          current_time=current_time)
        ...
        # fold, train_df, test_df, male_df, female_df, args, device, current_time)
        if args.debug:
            run_functional_mechanism_logistic_regression(fold=0, train_df=train_df, test_df=test_df, male_df=male_df,
                                                         female_df=female_df, args=args,
                                                         device=device,
                                                         current_time=current_time)
        else:
            for fold in range(args.folds):
                run_functional_mechanism_logistic_regression(fold=fold, male_df=male_df, female_df=female_df,
        ...
        # fold, train_df, test_df, male_df, female_df, args, device, current_time)
        if args.debug:
            run_fair_dpsgd_test(fold=0, train_df=train_df, test_df=test_df, male_df=male_df, female_df=female_df,
                                args=args,
                                device=device,
                                current_time=current_time)
        else:
